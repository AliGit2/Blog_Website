...
import { Poppins } from 'next/font/google';  // New Font Import
import Link from 'next/link';
import ShareButton from './shareBtn';
import SearchComponent from './SearchComponent';
import CustomDropdown from './CustomDropdown';

const poppins = Poppins({ subsets: ['latin'], weight: ['400', '600'] });  // Applying new font
    ...
    return (
        ...
        <div className='flex flex-col justify-center sm:items-center items-end relative sm:mb-4'>
            <div className="flex sm:items-start items-center justify-between w-full relative sm:px-5 px-2 sm:pb-14 sm:pt-8 pt-2">
                ...
                <Link href={"/"}>
                    <div className={`${poppins.className} md:text-6xl text-3xl tracking-tighter font-bold absolute left-1/2 transform -translate-x-1/2 text-center`}>
                        The News Discovery
                    </div>
                </Link>
            ...
            <div className="flex justify-end sm:justify-between sm:px-4 px-2 items-center w-full sm:border-t sm:border-b border-gray-300">
                ...
                <div className={`${poppins.className} sm:text-[20px] flex items-center  sm:space-x-10`}>
                    {items.map((item, i) => (
                ...
                {/* Mobile Menu */}
                {isMenuOpen && (
                    <div className={`${poppins.className} fixed top-0 left-0 h-full w-[65%] sm:w-[60%] bg-white dark:bg-neutral-950 dark:text-white text-black shadow-lg transition-transform transform translate-x-0 z-50`}>
                        <div className="flex justify-between items-center p-4 pt-5">
                            <span className={`${poppins.className} text-lg font-bold`}>The News Discovery</span>
                        ...
                        <ul className={`${poppins.className} p-4 space-y-4`}>
                            {items.map((item, i) => (
